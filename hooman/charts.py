...
def linechart(hapi, x, y, w, h, params):
    ...
    for t in range(options['ticks_x']+1):
        hapi.line(x+(t*(w//options['ticks_x'])), y+h, x+(t*(w//options['ticks_x'])), y+h+options['tick_size'])
        hapi.push_matrix()
        hapi.rotate(270)
    ...
    for i,d in enumerate(options['data']):
        ...
        try:
            ...
            x1 = hapi.constrain(d[0], options['range_x'][0], options['range_x'][1], x, x+w)
            y1 = 60+h-hapi.constrain(d[1], options['range_y'][0], options['range_y'][1], y, y+h)
            x2 = hapi.constrain(options['data'][i+1][0], options['range_x'][0], options['range_x'][1], x, x+w)
            y2 = 60+h-hapi.constrain(options['data'][i+1][1], options['range_y'][0], options['range_y'][1], y, y+h)
